...
Item {
    ...
    function setSetting(key, value) {
        ...
        writeSettings(`. + {"${key}": ${safeValue}}`);
    }

    function updateJsonBulk(dataObj) {
        writeSettings(`. + ${JSON.stringify(dataObj)}`);

        for (let key in dataObj) rawSettings[key] = dataObj[key];
    }

    // Aplica um filtro jq no settings.json. O flock serializa as escritas e o
    // mktemp dá um destino único: com um `.tmp` de nome fixo, dois writes
    // simultâneos gravavam no mesmo arquivo e a saída interleaved corrompia o
    // JSON — daí o jq seguinte falhava e deixava o `.tmp` órfão para trás.
    function writeSettings(jqFilter) {
        const filter = jqFilter.replace(/'/g, "'\\''");
        const cmd = `mkdir -p "$(dirname '${settingsJsonPath}')"; ` +
                    `exec 9>'${settingsJsonPath}.lock'; flock 9; ` +
                    `[ -f '${settingsJsonPath}' ] || echo '{}' > '${settingsJsonPath}'; ` +
                    `tmp=$(mktemp --suffix=.tmp '${settingsJsonPath}.XXXXXX'); ` +
                    `jq '${filter}' '${settingsJsonPath}' > "$tmp" && ` +
                    `mv "$tmp" '${settingsJsonPath}' || rm -f "$tmp"`;
        sh(cmd);
    }

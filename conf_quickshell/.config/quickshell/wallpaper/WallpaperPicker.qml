...
Item {
            ...
            if (alreadyExists) {
                ...
                const downloadScript = `
                    ...
                    if [ -n "$URL" ]; then
                        ...
                        STEP=$(shuf -i 1-90 -n 1)
                        CHOSEN=$(printf "wipe\\nany" | shuf -n 1)
                        if [ "$CHOSEN" = "wipe" ]; then
                            ANGLE=$(shuf -i 0-365 -n 1)
                            TRANS_ARGS="--transition-type wipe --transition-step $STEP --transition-angle $ANGLE"
                        else
                            TRANS_ARGS="--transition-type any --transition-step $STEP"
                        fi
    ...
    readonly property string srcDir: {
        ...
        return (dir && dir !== "") 
        ? dir 
        : Quickshell.env("HOME") + "/wallpapers/desktop/SPKyjw9HRTImK90/8mWeerfxH3HwuPa/jOsyc1nZGsaPUXw"
    }
        ...
        delegate: Item {
            ...
            Item {
                ...
                Item {
                    ...
                    // MediaPlayer removed to avoid QtMultimedia dependency
                    // VideoOutput removed to avoid QtMultimedia dependency

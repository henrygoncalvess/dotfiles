...
Item {
    ...
    readonly property var filtered: {
        ...
        for (var i = 0; i < apps.length; i++) {
            var a = apps[i]
            var s = root.fuzzyScore(q, (a.name || "").toLowerCase())
            if (s < 0 && a.categories)
                s = root.fuzzyScore(q, a.categories.toLowerCase())
            if (s >= 0)
                scored.push({ a: a, s: s, n: (a.name || "").toLowerCase() })
        }
        scored.sort(function(x, y) {
            return x.s !== y.s ? x.s - y.s : (x.n < y.n ? -1 : 1)
        })
        return scored.map(function(o) { return o.a })
    }

    // Fuzzy: substring (rankeado pela posição) e, se não casar, subsequência
    // (chars na ordem, permite lacunas). Retorna -1 se não casa; menor = melhor.
    function fuzzyScore(q, text) {
        ...
        return qi < q.length ? -1 : 1000 + first
    }

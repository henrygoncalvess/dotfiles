...
PanelWindow {
    ...
    // Position slightly below top bar (8px gap)
    margins.top: Theme.notchHeight + 8
	...
	Item {
		...
		Rectangle {
			...
			// To glue it to the right wall perfectly, we remove the right radius
			Rectangle {
			    anchors.right: parent.right
			    anchors.top: parent.top
				...
				Row {
					...
					Item {
						...
						Image {
							...
							anchors.fill: parent
							source: {
								if (root.current?.image && root.current.image !== "") {
									return root.current.image
								}

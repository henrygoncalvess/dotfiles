...
STOW_PACKAGES=(frigate conf_home conf_wall conf_posh conf_code conf_git conf_shell conf_kitty conf_rofi conf_quickshell conf_qylock conf_nvim)
...
# conf_hypr: o Omarchy gerencia ~/.config/hypr como sua árvore de config
# primária. `omarchy update` roda `omarchy-refresh-config` que faz `cp -f` do
# default sobre o que estiver lá. Se o stow tiver dobrado o diretório inteiro
# num symlink pro repo, o `cp -f` atravessa o link e sobrescreve o arquivo
# versionado — daí o erro "this file is autogenerated" e a perda das nossas
# customizações. Com --no-folding o diretório fica real (do Omarchy) e os nossos
# arquivos são links individuais; o refresh do Omarchy substitui o link por um
# arquivo real, e o próximo `import_conf.sh` recria o link sem estragar nada.
OVERLAY_PACKAGES=(conf_omarchy conf_walker conf_systemd conf_qs-vpets conf_easyeffects conf_pipewire conf_autostart conf_hypr)
